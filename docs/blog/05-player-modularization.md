...
`play`, `pause`, `seek`, `stop`이 모두 `async throws`인 이유는 엔진 명령이 단순 메모리 플래그 변경이 아니라 vendor SDK, `AVPlayer`, 렌더 surface, DRM 준비 상태와 만나는 작업이기 때문이다. 실패 가능성을 protocol 표면에 올려 두면 `PlayerCore`가 모든 엔진 명령을 같은 방식으로 직렬화하고, 실패를 `PlaybackState.failed`와 `PlayerEvent.didFail`로 정규화할 수 있다. `stop`은 사용자 종료, 재생 완료, 시스템 정리처럼 의미가 다른 종료를 구분하기 위해 `PlayerStopReason`을 받는다. 엔진의 상태 출력은 `PlaybackState`를 직접 소유하는 대신 `PlayerEngineOutput` 스트림으로만 나온다. Core가 이 출력을 reducer에 넣어 최종 `PlaybackState`를 만든다.
...
ViewModel이나 Shell 계층은 `PlaybackCommand`를 던지고, `PlayerCore`가 내보내는 `AsyncStream<PlaybackState>`를 `PlayerStateBinder` 같은 배선 객체로 구독한다. 그 사이에 양방향 추가 채널은 없다. 엔진이 "자기 마음대로 UI를 그리는" 경로는 의도적으로 차단했다. 이렇게 하면 디버깅 지점이 줄어든다. 화면이 이상하면 마지막 들어간 command와 마지막 나온 state를 먼저 확인하면 된다.
...
```mermaid
flowchart LR
    Shell[Player Shell UI] -->|PlaybackCommand| Core[PlayerCore actor]
    Core --> Engine[Engine Adapter actor]
    Engine -->|outputStream| Core
...
.product(name: "VideoPlayerSkin",          ...),    // 재사용 플레이어 UI
```
...
- **product 선택**: SwiftPM 소비자가 필요한 라이브러리 product만 링크하게 만드는 경계다. 현재 패키지는 umbrella product 없이 5개 product를 직접 노출한다.
...
videoplayer-ios-ms/
├── Sources/
│   ├── VideoPlayerCore/          # 도메인 + 상태 머신 (SDK/UIKit 모름)
│   │   ├── Domain/               # PlaybackSource, Command, State, Event
│   │   ├── Contract/             # PlayerPlaybackEngine, PlayerEngineOutput
│   │   ├── StateTransition/      # PlaybackStateReducer
│   │   ├── Internal/             # PlayerCore actor
│   │   └── Download/             # PlayerDownloadCenter, DownloadedContent
│   │
│   ├── VideoPlayerShellSupport/  # 조립, render surface, lifecycle/audio
│   ├── VideoPlayerEngineNative/  # AVPlayerAdapter
│   ├── VideoPlayerEngineKollus/  # Kollus adapter + storage/download/mapper
│   └── VideoPlayerSkin/          # 재사용 플레이어 UI
│
...
이 디렉토리를 가만히 들여다보면 다섯 원칙이 그대로 보인다. `VideoPlayerCore/Domain/`은 어떤 vendor도 모르고, 엔진은 `VideoPlayerEngineNative/`와 `VideoPlayerEngineKollus/`로 분리되어 있고, `Vendor/`와 `Binaries/`는 격리되어 있고, `actor`가 박힌 파일은 `VideoPlayerCore/Internal/PlayerCore.swift`다. **원칙이 곧 폴더 이름이다.**

특히 흥미로운 부분이 `Packaging/Kollus/Stub/`이다. 시뮬레이터에서 빌드만이라도 통과시키기 위해 만든 stub SPM 패키지다. 시뮬레이터에는 KollusSDK 실제 바이너리가 동작하지 않으니, 시뮬레이터용 slice는 stub 라이브러리로 대체한다. 그 stub은 실제로 동작하지 않지만 link에는 성공한다. 이 트릭이 없으면 시뮬레이터 빌드가 통째로 깨진다. vendoring 디테일이 별도 문서(`docs/kollus/kollus-sdk-vendoring.md`)에 정리될 만큼 정교하게 다뤄지는 이유다.

<details>
<summary>용어 토글: 디렉토리 구조를 읽는 법</summary>

- **`VideoPlayerCore/Domain`**: vendor SDK와 UI를 모르는 순수 도메인 값 타입이 놓이는 곳이다.
- **`VideoPlayerCore/Internal`**: 외부에 직접 노출하지 않는 내부 상태 머신과 actor 구현이 놓이는 곳이다.
- **`VideoPlayerShellSupport`**: host가 엔진, core, render surface, lifecycle 처리를 조립하는 경계다.
- **`VideoPlayerEngineNative`**: Apple `AVPlayer` 기반 구현이다. DRM vendor SDK 없이 동작한다.
- **`VideoPlayerEngineKollus`**: Kollus SDK 호출을 우리 도메인 인터페이스로 번역하는 구현이다.
- **`VideoPlayerSkin`**: Core/ShellSupport만 알고 엔진은 모르는 재사용 UI다.
...
넷째, **책임의 명확성**. 비대한 ViewController에 섞여 있던 책임이 protocol, core, engine adapter, shell support, skin으로 갈라졌다. 자막 로직을 고치고 싶은 사람은 자막 관련 protocol과 Skin 렌더링을 먼저 보면 된다. 백그라운드 오디오 정책을 바꾸고 싶은 사람은 `PlayerAudioSessionManager`와 lifecycle coordinator를 보면 된다. 새 팀원이 합류했을 때 "이 코드 어디서부터 봐야 해요?"라는 질문에 더 짧게 답할 수 있다.
...
- **간접화**: 직접 호출하지 않고 protocol, core, adapter, shell support 같은 중간 계층을 거쳐 호출하는 구조다.
...
가까운 미래에는 두 가지가 있다. 하나는 **Swift Testing 기반 회귀망 강화**. 마이그레이션 자체는 끝났고, 이제는 `confirmation`을 활용한 비동기 스트림 검증과 host/Example 빌드 검증을 더 촘촘히 묶는 일이 남아 있다. 또 하나는 **Kollus coverage 확장 검증**. PiP, 외부 출력, HLS ABR, 라이브 상태, 다음 회차 이벤트 같은 표면은 API로 드러나기 시작했지만, 실제 강의 플로우에서 어떤 기능을 켜고 어떤 기능을 Shell 정책으로 막을지 정착시키는 작업이 남아 있다.
...
flowchart LR
    ...
    Near --> SwiftTesting[Swift Testing 회귀망 강화]
...
- 사내 코드: `Package.swift`, `Sources/VideoPlayerCore/`, `Sources/VideoPlayerShellSupport/`, `Sources/VideoPlayerEngineKollus/`, `Sources/VideoPlayerSkin/`, `Tests/VideoPlayerModuleTests/Support/`
