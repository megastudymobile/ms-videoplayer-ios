...
| `KollusSeekPreviewSource` | 시킹 프리뷰 썸네일이 전 프레임 스프라이트 시트 1장 | 파일명(`WxHxN`) 파싱 → 타일 crop. **전용 actor**에서 비트맵 강제 디코드(lazy decode가 메인을 막는 것 방지), prepare-ready/`.thumbnailReady`에 워밍업 |
