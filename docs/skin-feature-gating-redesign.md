...
> "어느 feature일 때 노출"은 블록 자신이 `requiredFeatures`로 선언하고
...
flowchart LR
    ...
    subgraph BlockSide["블록 쪽 (신규 — 같은 정신)"]
        B["BookmarkButtonBlock"] -->|"requiredFeatures = .bookmarks 선언"| P2["apply → 노출 게이팅<br/>'나는 .bookmarks 있어야 보인다'"]
    end
```

엔진이 `EngineBookmarkAbility`를 채택하는 것만으로 능력을 선언하듯(§1의 probe),
블록은 `requiredFeatures` 프로퍼티 하나로 필요 feature를 선언한다.
"선언만 하면 협상은 시스템이 한다"는 정신이 계약의 양 끝에서 반복되니
새로 배울 개념이 없다.
...
flowchart TB
    ...
    subgraph Example["Example (host)"]
        BBB["BookmarkButtonBlock<br/>requiredFeatures = [.bookmarks] 선언"]
...
| **어느 feature일 때 보이나?** | **블록 타입** | **`requiredFeatures` 선언 ← 신규** |
...
### 4-2. `PlayerSkinBlock.requiredFeatures` — 블록이 필요 feature를 선언 (패키지)
...
public protocol PlayerSkinBlock: AnyObject {
    ...
    /// - 빈 집합이 아닌 값을 선언한 블록은 view.isHidden 을 직접 제어하지 않는다 —
...
| 빈 집합 엣지 케이스 | — | 채택+`[]` 선언 시 이상 동작 | **없음 — `[]` = 조건 없음** |
...
//  VideoPlayerExample
//
//  북마크 버튼 — requiredFeatures 선언으로 .bookmarks 지원 엔진에서만
...
extension PlayerSkinBlueprint {
    ...
    static var example: PlayerSkinBlueprint {
        ...
        // 북마크 — 노출 조건은 블록 타입이 requiredFeatures 로 선언.
...
/// 조건 선언 probe — requiredFeatures 를 주입받아 override 한다.
@MainActor
...
flowchart LR
    ...
    subgraph After["개선 후 — 선언 1곳 + 동작 1곳"]
        direction TB
        a1["BookmarkButtonBlock 타입 선언<br/>requiredFeatures = .bookmarks 선언"]
...
- **엣지 케이스 소멸.** 분리안은 "채택했는데 `[]` 선언"라는 이상 상태가 가능
...
1. **조건이 타입과 한 몸** — 블록 파일만 보면 그 블록의 노출 조건이 보이고,
   등록 지점에서 조건을 빼먹는 실수가 원천 차단된다. "선언만 하면 협상은
